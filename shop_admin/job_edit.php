...
// 求人情報の取得
$job = $db->fetch(
    "SELECT * FROM jobs WHERE id = ? AND shop_id = ?",
    [$job_id, $shop_id]
);
...
// 求人更新処理
if ($_POST && isset($_POST['update_job'])) {
        ...
        try {
            $db->query(
                "UPDATE jobs SET title = ?, description = ?, requirements = ?, benefits = ?, 
                                work_hours = ?, salary_min = ?, salary_max = ?, location = ?, 
                                status = ?, updated_at = NOW() 
                 WHERE id = ? AND shop_id = ?",
                [$title, $description, $requirements, $benefits, $work_hours, 
                 $salary_min, $salary_max, $location, $status, $job_id, $shop_id]
            );
            
            $_SESSION['success_message'] = '求人情報を更新しました。';
            header('Location: jobs.php');
            exit;
            
        } catch (Exception $e) {
            $errors[] = '求人更新中にエラーが発生しました: ' . $e->getMessage();
        }
    }
}
                        ...
                        <form method="POST" class="needs-validation" novalidate>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="title" 
                                       ...
                                       value="<?php echo htmlspecialchars($_POST['title'] ?? $job['title']); ?>" required>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="4" required
                                          placeholder="具体的な仕事内容を詳しく記載してください"><?php echo htmlspecialchars($_POST['description'] ?? $job['description']); ?></textarea>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="requirements" name="requirements" rows="3"
                                          placeholder="年齢、経験、資格などの条件があれば記載してください"><?php echo htmlspecialchars($_POST['requirements'] ?? $job['requirements']); ?></textarea>
                            </div>

                            <div class="mb-3">
                                <label for="benefits" class="form-label">福利厚生・特典</label>
                                <textarea class="form-control" id="benefits" name="benefits" rows="3"
                                          placeholder="交通費支給、食事補助、研修制度など"><?php echo htmlspecialchars($_POST['benefits'] ?? $job['benefits']); ?></textarea>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="work_hours" name="work_hours" 
                                       ...
                                       value="<?php echo htmlspecialchars($_POST['work_hours'] ?? $job['work_hours']); ?>" required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control" id="salary_min" name="salary_min" 
                                               ...
                                               value="<?php echo htmlspecialchars($_POST['salary_min'] ?? $job['salary_min']); ?>">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control" id="salary_max" name="salary_max" 
                                               min="0" step="1" required
                                               value="<?php echo htmlspecialchars($_POST['salary_max'] ?? $job['salary_max']); ?>">
                                        <span class="input-group-text">円</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select" id="status" name="status">
                                    <option value="active" <?php echo ($_POST['status'] ?? $job['status']) === 'active' ? 'selected' : ''; ?>>公開する</option>
                                    <option value="inactive" <?php echo ($_POST['status'] ?? $job['status']) === 'inactive' ? 'selected' : ''; ?>>非公開</option>
                                    <option value="closed" <?php echo ($_POST['status'] ?? $job['status']) === 'closed' ? 'selected' : ''; ?>>終了</option>
                                </select>
                            </div>

                            <div class="d-grid gap-2">
                                <button type="submit" name="update_job" class="btn btn-warning btn-lg">
