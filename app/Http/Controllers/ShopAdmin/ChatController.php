...
class ChatController extends Controller
{
    ...
    public function index()
    {
        ...
        $chatRooms = ChatRoom::where('shop_id', $shopId)
            ->with(['user', 'application.job'])
        ...
        return view('shop-admin.chat.index', compact('chatRooms'));
    }
    ...
    public function show(Request $request, $id = null)
    {
        ...
        if ($applicationId || $request->has('application_id')) {
            $applicationId = $applicationId ?? $request->get('application_id');
            $application = Application::where('id', $applicationId)
                ->whereHas('job', function ($query) use ($shopId) {
            ...
            if (!$application) {
                return redirect()->route('shop-admin.applications.index')
            ...
            if (!$room) {
                ...
                $room->load(['user', 'application.job']);
            }
        } else if ($roomId) {
            ...
            if (!$room) {
                return redirect()->route('shop-admin.chat.index')
        ...
        if ($request->isMethod('post') && $request->input('action') === 'send_message') {
            return $this->sendMessage($request, $room, $shopAdmin);
        }
        ...
        return view('shop-admin.chat.show', compact('room', 'messages'));
    }

    /**
     * メッセージを送信
     */
    private function sendMessage(Request $request, ChatRoom $room, $shopAdmin)
    {
        ...
        try {
            ...
            ChatMessage::create([
                'room_id' => $room->id,
                'sender_type' => 'shop_admin',
                'sender_id' => $shopAdmin->id,
                'message' => $message,
            ...
            return redirect()->route('shop-admin.chat.show.id', ['id' => $room->id])
